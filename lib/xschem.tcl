...
proc ::analog_lens::run_op {} {
    ...
    set run_log {}; set run_started [clock seconds]
    set previous [pwd]
    try {
        cd $directory
...
proc ::analog_lens::run_readable {} {
    ...
    if {$failed || ![file exists $run_file] || [file size $run_file] == 0 || [regexp -nocase {(?m)^(fatal error|error:|doanalyses:)} $run_log]} {
...
proc ::analog_lens::export_report {filename} {
    ...
    set out [csv_row {instance model family hierarchy width length fingers multiplier sample dataset id_A gm_S gds_S gmid_1_V intrinsic_gain ro_ohm vgs_V vds_V headroom_V cgg_total_F ft_est_Hz status notes raw_file}]
    append out \n
    foreach r $records {
        set v [get $r values]
